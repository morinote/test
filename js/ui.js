...
export function adjustAllInputWidths() {
  document
    .querySelectorAll(".form-input--numeric, .dynamic-section__header input")
...
export function initializeTables() {
  ...
  document.getElementById("calculation-table-container").innerHTML =
    createGenericTableHTML("calculation-table", []);
  document.getElementById("beer-server-table-container").innerHTML =
    createGenericTableHTML("beer-server-calculation-table", [
      "参加者",
      "ビアサーバー料金",
      "支払合計",
      "一人当たり",
  ...
  document.getElementById("food-table-container").innerHTML =
    createGenericTableHTML("food-calculation-table", [
      "参加者",
      "食材料金",
      "支払合計",
      "一人当たり",
...
export function renderParticipants() {
  const lists = {
    main: {
      container: document.getElementById("participants-list"),
      key: "mainチェック",
    },
    beer: {
      container: document.getElementById("beer-server-participants-list"),
      key: "ビアサーバーチェック",
    },
    food: {
      container: document.getElementById("food-participants-list"),
      key: "食材チェック",
    },
  };
...
function updateTables() {
  const selectedParticipants = Array.from(
    document.querySelectorAll("#participants-list input:checked")
  ).map((cb) => cb.value);
  const selectedBeerServerParticipants = Array.from(
    document.querySelectorAll("#beer-server-participants-list input:checked")
  ).map((cb) => cb.value);
  const selectedFoodParticipants = Array.from(
    document.querySelectorAll("#food-participants-list input:checked")
  ).map((cb) => cb.value);
  ...
  updateParticipantRows(
    ...
    (clone, p) => {
      ...
      clone
        .querySelectorAll("tr")
        .forEach((row) => (row.dataset.participant = p));
      ...
      clone.querySelector(".fee-input").value =
...
function updateSummaryTable(results) {
  ...
  if (dynamicSections) {
    dynamicSections.forEach((sec) =>
      headers.push(sec.name.replace(/料金$/, ""))
    );
  }
  headers.push("最終収支");
...
function updateUIWithResults(results) {
  ...
  document
    .querySelectorAll("#calculation-table .calculation-table__header-row")
    .forEach((headerRow) => {
      const p = headerRow.dataset.participant;
      if (calculationResults[p]) {
        const valueRow =
          headerRow.nextElementSibling.nextElementSibling.nextElementSibling;
        if (valueRow) {
          valueRow.querySelector(".total-payment").textContent =
            formatNumberWithCommas(calculationResults[p].totalPayment);
          valueRow.querySelector(".per-person-payment").textContent =
            formatNumberWithCommas(Math.round(perPersonExpense));
          updateBalanceCell(
            valueRow.querySelector(".balance"),
...
function updateStandardTableUI(
  ...
  document.querySelectorAll(`${tableSelector} tbody tr`).forEach((row) => {
    ...
    if (results[p] && results[p][balanceKey] !== undefined) {
      row.querySelector(".total-payment").textContent = formatNumberWithCommas(
        results[p][paymentKey]
      );
      row.querySelector(".per-person-payment").textContent =
        formatNumberWithCommas(Math.round(perPerson));
...
function updateBalanceCell(cell, balanceValue) {
  ...
  if (roundedBalance > 0) {
    ...
    cell.textContent = `払う: ${formatNumberWithCommas(
...
function createNewSection() {
  ...
  const dynamicSectionsContainer = document.getElementById(
  ...
  document.getElementById(`${sectionId}-table-container`).innerHTML =
    createGenericTableHTML(`${sectionId}-calculation-table`, [
      "参加者",
      sectionNameForData,
      "支払合計",
      "一人当たり",
...
function updateDynamicSectionTable(section) {
  ...
  const selectedParticipants = Array.from(
    section.querySelectorAll(`#${sectionId}-participants-list input:checked`)
  ).map((cb) => cb.value);
  ...
  updateParticipantRows(
    ...
    (clone, p) => {
      ...
      if (!participantInputValues[p][section.dataset.sectionName])
        participantInputValues[p][section.dataset.sectionName] = 0;
...
function handleTableInput(event) {
  ...
  if (!(target instanceof Element) || !target.classList.contains("form-input"))
    return;

  const participant = target.closest("[data-participant]")?.dataset.participant;
  let type;

  if (target.classList.contains("ticket-quantity-input"))
    type = target.dataset.ticketType;
  else if (target.classList.contains("fee-input")) type = "手数料";
  else if (target.classList.contains("beer-server-input"))
    type = "ビアサーバー";
...
export function setupEventListeners() {
  ...
  document
    ...
    .addEventListener("click", createNewSection);
  ...
  document.addEventListener(
    ...
    (event) => {
      ...
      if (
        ...
        event.target.matches('input[type="number"].form-input')
      ) {
